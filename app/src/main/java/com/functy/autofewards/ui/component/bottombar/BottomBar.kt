...
data class NavigationBadgeState(
    val accountConfiguredCount: Int = 0,
...
internal fun badgeFor(index: Int, state: NavigationBadgeState): NavBadge? = when (index) {
    BottomBarDestination.Account.ordinal ->
        state.accountConfiguredCount.takeIf { it > 0 }?.let { NavBadge(it, BadgeTone.Accent) }

    BottomBarDestination.Bing.ordinal ->
        if (state.bingRunning) NavBadge(1, BadgeTone.Alert) else null
